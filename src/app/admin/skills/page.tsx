    ...
    fetch("/api/profile")
      ...
      .then((d) => {
        if (d.success && d.data?.sections) {
          setSkillsVisible(d.data.sections.skills ?? true);
        }
      })
      .catch(() => {});
  }, []);

  const handleToggleSkillsVisibility = async () => {
    const nextVal = !skillsVisible;
    setSkillsVisible(nextVal);
    setTogglingVisibility(true);
    try {
      await fetch("/api/profile", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sections: { skills: nextVal } }),
      });
      setMessage({
        type: "success",
        text: `Skills section is now ${nextVal ? "visible on" : "hidden from"} public portfolio.`,
      });
    } catch {
      setSkillsVisible(!nextVal);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 self-start sm:self-auto">
          ...
          <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl glass border border-border">
            <span className={`text-[11px] font-bold ${skillsVisible ? "text-emerald-500 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}>
              {skillsVisible ? "Public: Live" : "Public: Hidden"}
            </span>
            {!skillsVisible && (
              <button
                type="button"
                disabled={togglingVisibility}
                onClick={handleToggleSkillsVisibility}
            ...
            <label className="relative inline-flex items-center cursor-pointer select-none">
              <input
                ...
                disabled={togglingVisibility}
                onChange={handleToggleSkillsVisibility}
